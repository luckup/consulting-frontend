...
import { getNewsHeadlines } from '@/i18n/localized/news'

export function HomeLatestNewsStrip() {
  const { locale, t } = useI18n()
  const headlines = getNewsHeadlines(locale)
  const featured = headlines[0]
  const more = headlines.slice(1)
  const { news } = getMessages(locale).homeSections
    ...
    <AnimatedHomeSection id="insights" className="scroll-mt-[100px]">
      <RevealItem>
        <HomeSectionHeader
          ...
          action={{ to: '/news', label: news.action }}
        />
      </RevealItem>
